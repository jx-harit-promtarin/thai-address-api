...
| ------ | ---- | -------- | -------------- |
| GET | `/api/v1/continents` | รายการทวีป | `continent` |
| GET | `/api/v1/countries` | รายการประเทศ | `country` |
...
| -------- | ----------- | -------- |
| `/api/v1/continents` | `id` | filter ทวีปตาม id |
| | `code` | filter ทวีปตามโค้ด (AS, EU, AF, NA, SA, OC, AN) |
| `/api/v1/countries` | `id` | filter ประเทศตาม id (ISO 3166-1 numeric) |
| | `iso_alpha2` | filter ประเทศตามโค้ด ISO alpha-2 (เช่น TH) |
| | `iso_alpha3` | filter ประเทศตามโค้ด ISO alpha-3 (เช่น THA) |
| | `continent_id` | filter ประเทศตาม id ของทวีป |
| | `continent_name_th` | filter ประเทศตามชื่อทวีป (ไทย) |
| | `continent_name_en` | filter ประเทศตามชื่อทวีป (อังกฤษ) |
| `/api/v1/geographies` | `id` | filter ภูมิภาคตาม id |
| | `country_id` | filter ภูมิภาคตาม id ของประเทศ |
| | `country_name_th` | filter ภูมิภาคตามชื่อประเทศ (ไทย) |
| | `country_name_en` | filter ภูมิภาคตามชื่อประเทศ (อังกฤษ) |
...
```bash
# รายการทวีป / ประเทศทั้งหมด
curl "http://localhost:8080/api/v1/continents"
curl "http://localhost:8080/api/v1/countries"

# ประเทศตามโค้ด ISO / ประเทศในทวีปที่ระบุ
curl "http://localhost:8080/api/v1/countries?iso_alpha2=TH"
curl "http://localhost:8080/api/v1/countries?continent_name_en=Asia"
...
| `ALLOW_ORIGIN` | ❌ | ค่า `Access-Control-Allow-Origin` สำหรับ CORS (ค่าเริ่มต้น `*` = ทุก origin) |
| `RATE_LIMIT_RPS` | ❌ | จำนวน request ต่อวินาทีต่อ IP (ค่าเริ่มต้น `5`) |
| `RATE_LIMIT_BURST` | ❌ | จำนวน request ที่ยิงพร้อมกันได้ต่อ IP (ค่าเริ่มต้น `10`) |

หาก `SUPABASE_URL` หรือ `SUPABASE_KEY` ไม่ถูกตั้งค่า server จะไม่ start

### CORS และ Rate limiting

- **CORS**: ทุก endpoint ส่ง CORS headers และตอบ preflight (`OPTIONS`) ให้ frontend เรียกข้าม origin ได้
  ค่าเริ่มต้นเปิดให้ทุก origin (`*`) ปรับจำกัดเฉพาะ domain ได้ผ่าน `ALLOW_ORIGIN`
- **Rate limiting**: จำกัด request แบบ token bucket **ต่อ IP** (อ่าน IP จริงจาก `X-Forwarded-For`)
  เกินโควตาจะได้ `429 Too Many Requests` พร้อม header `Retry-After` ปรับได้ผ่าน `RATE_LIMIT_RPS` / `RATE_LIMIT_BURST`

⚠️ Rate limit เป็นแบบ in-memory จึงนับ **ต่อ instance** — บน Cloud Run ที่ scale หลาย instance
ลิมิตรวมจริงจะเท่ากับ `RATE_LIMIT_RPS × จำนวน instance` หากต้องการลิมิตแบบ global ต้องใช้ store ภายนอก (เช่น Redis)

> ปัจจุบัน server อ่านค่าจาก environment variable ของระบบเท่านั้น
> (โค้ดโหลดไฟล์ `.env` ด้วย `godotenv` ถูก comment ไว้ใน `main.go`)
> หากต้องการใช้ไฟล์ `.env` ให้ uncomment ส่วนนั้นและเพิ่ม dependency `github.com/joho/godotenv`
...
> ⚠️ หากใช้ไฟล์ `.env` อย่า commit ขึ้น git — ควรเพิ่ม `.env` ไว้ใน `.gitignore`
...
```bash
# กำหนดผ่าน environment variable แล้วรัน
export SUPABASE_URL="https://xxxx.supabase.co"
export SUPABASE_KEY="your-anon-key"
go run .
```
...
- Go 1.25 ขึ้นไป (สำหรับรันแบบ local)
